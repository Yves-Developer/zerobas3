...
/**
 * Portable ZeroBase SDK
 */

export class ZeroBaseTable<T = any> {
  constructor(
...
export class ZeroBaseAuth {
    ...
    async signInWithGoogle(callbackURL?: string) {
        return await this.authClient.signIn.social({
            provider: "google",
            callbackURL: callbackURL || (typeof window !== 'undefined' ? window.location.origin + "/dashboard" : "")
        });
    }
